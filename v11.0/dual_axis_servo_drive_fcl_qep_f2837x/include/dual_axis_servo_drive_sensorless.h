...
// esmoLostLock[x] latches 1 if the sensorless closed loop loses lock during a
// run; read it once per test point (1 = grid cell outside the safe region).
extern volatile float32_t esmoLockAngErrThrDeg[2];
extern volatile float32_t esmoLockSpdErrThrPu[2];
extern volatile uint16_t  esmoLockDebounceMs[2];
extern volatile uint16_t  esmoLostLock[2];
extern volatile float32_t esmoLockAngErrDeg[2];
extern void updateSensorlessLockWatch(struct _MOTOR_Vars_t_ *pMotor,
                                       uint16_t motorIdx);
