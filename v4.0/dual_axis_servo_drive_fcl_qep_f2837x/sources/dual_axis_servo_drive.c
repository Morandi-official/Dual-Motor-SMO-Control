...
// eSMO startup transient capture. Compact Q15 log, about 12s at 8Hz on M1.
#define ESMO_STARTUP_LOG_SIZE       96U
#define ESMO_STARTUP_LOG_RATE_HZ    8U
...
void main(void)
{
    ...
    // ��ʼ��Эͬ�ٶ� PI ���������� (����ֵ�ɸ���ʵ���������)
    pid_sync_spd.param.Kp   = 0.0;
    pid_sync_spd.param.Ki   = 0.000;
    pid_sync_spd.param.Kd   = 0.0;
    pid_sync_spd.param.Kr   = 1.0;
    pid_sync_spd.param.Umax = 0.5;   // ����Эͬ�������������������
    pid_sync_spd.param.Umin = -0.5;
...
// -----------------------------------------------------------------------------
            ...
            (isSensorlessTakeoverActive(&motorVars[0]) != 0U))
    {
        ...
        motorVars[0].pid_spd.data.i1 =
                (isSensorlessTakeoverActive(&motorVars[0]) != 0U) ?
                        (getSensorlessStartupIqRef(&motorVars[0]) /
                                motorVars[0].pid_spd.param.Kp) :
                        0;
        ...
        motorVars[0].pid_spd.term.Out =
                (isSensorlessTakeoverActive(&motorVars[0]) != 0U) ?
                        getSensorlessStartupIqRef(&motorVars[0]) : 0;
    }
    ...
    motorVars[0].ptrFCL->pi_iq.ref =
            (motorVars[0].ptrFCL->lsw == ENC_ALIGNMENT) ? 0 :
                    ((motorVars[0].ptrFCL->lsw == ENC_WAIT_FOR_INDEX) ||
                     (isSensorlessTakeoverActive(&motorVars[0]) != 0U)) ?
                            getSensorlessStartupIqRef(&motorVars[0]) :
                            (motorVars[0].pid_spd.term.Out + syncPI_out);
...
// -----------------------------------------------------------------------------
            ...
            (isSensorlessTakeoverActive(&motorVars[1]) != 0U))
    {
        ...
        motorVars[1].pid_spd.data.i1 =
                (isSensorlessTakeoverActive(&motorVars[1]) != 0U) ?
                        (getSensorlessStartupIqRef(&motorVars[1]) /
                                motorVars[1].pid_spd.param.Kp) :
                        0;
        ...
        motorVars[1].pid_spd.term.Out =
                (isSensorlessTakeoverActive(&motorVars[1]) != 0U) ?
                        getSensorlessStartupIqRef(&motorVars[1]) : 0;
    }
    ...
    motorVars[1].ptrFCL->pi_iq.ref =
            (motorVars[1].ptrFCL->lsw == ENC_ALIGNMENT) ? 0 :
                    ((motorVars[1].ptrFCL->lsw == ENC_WAIT_FOR_INDEX) ||
                     (isSensorlessTakeoverActive(&motorVars[1]) != 0U)) ?
                            getSensorlessStartupIqRef(&motorVars[1]) :
                            (motorVars[1].pid_spd.term.Out - syncPI_out);
