...
static inline void captureESMOStartupLog(MOTOR_Vars_t *pMotor,
                                         uint16_t motorNum);
static inline void captureESMOCompareLog(MOTOR_Vars_t *pMotor,
                                         uint16_t motorNum);
...
// eSMO/QEP long-window compare log. At 10Hz and 96 points it records 9.6s.
#define ESMO_COMPARE_LOG_ENABLE      1U
#define ESMO_COMPARE_LOG_SIZE        96U
#define ESMO_COMPARE_LOG_RATE_HZ     10U
...
volatile uint16_t esmoCompareLogArmed = 1;
...
static float32_t esmoQepAnglePrevPu[2] = {0.0f, 0.0f};
                                          ...
                                          uint16_t motorNum)
{
    ...
    {
        esmoQepAnglePrevPu[motorNum] = qepAngle;
    ...
    angleDelta = wrapPuHalfLocal(qepAngle - esmoQepAnglePrevPu[motorNum]);
    esmoQepAnglePrevPu[motorNum] = qepAngle;

    qepSpeed = angleDelta / (pMotor->baseFreq * pMotor->Ts);
    qepSpeed = __fsat(qepSpeed, 1.0f, -1.0f);
                                         ...
                                         uint16_t motorNum)
{
    ...
    esmoCompareLog[esmoCompareLogIndex].qepSpeed_q15 =
            q15Log((motorNum <= MTR_2) ? esmoQepSpeedPu[motorNum] : 0.0f);
    esmoCompareLog[esmoCompareLogIndex].speedErr_q15 =
            q15Log(pMotor->esmoSpeedErrPu);
