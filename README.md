...
| 速度点 | 稳态段 | 速度指令 / pu | eSMO均速 / pu | QEP均速 / pu | 速度误差RMS / pu | 速度误差峰值 / pu | 角度平均偏差 / 电角度 | 去偏置角度RMS / 电角度 | iqRef峰值 / pu | iqFbk峰值 / pu | eqMag均值 / pu |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 0.1 pu | 0–95 | 0.09998 | 0.10002 | 0.10003 | 0.00688 | 0.01544 | -16.53 | 1.23 | 0.04367 | 0.05298 | 0.08645 |
| 0.2 pu | 0–95 | 0.19999 | 0.19995 | 0.19988 | 0.00273 | 0.00681 | 11.38 | 0.67 | 0.04828 | 0.04938 | 0.14933 |
| 0.3 pu | 0–95 | 0.29997 | 0.29983 | 0.29984 | 0.00255 | 0.00546 | 0.20 | 0.58 | 0.05435 | 0.05603 | 0.21106 |
| 0.4 pu | 0–95 | 0.39998 | 0.39971 | 0.39965 | 0.00203 | 0.00470 | -5.21 | 0.52 | 0.06107 | 0.06296 | 0.27222 |
| 0.5 pu | 0–95 | 0.49998 | 0.50044 | 0.50014 | 0.01257 | 0.01947 | -22.46 | 0.88 | 0.07627 | 0.08322 | 0.30267 |

分析结论：

1. 0.1 pu、0.5 pu 下的 `angleErr` 均值偏大，而 0.3 pu、0.4 pu 下角度平均偏差较小，说明当前角度补偿并不是全速度范围统一最优，而是存在明显的速度相关相位偏差。也就是说，即使 QEP 旁路零位已经按 FOC 零位重新统一，eSMO final angle 与 QEP 参考角之间仍可能随速度变化产生残余相位差。

2. 从 `angleErr` 去偏置 RMS 看，0.2–0.4 pu 的短时角度抖动较小，约 0.52–0.67 电角度；0.1 pu 和 0.5 pu 略大，分别约 1.23 和 0.88 电角度。这说明中速段角度观测更稳定，低速段受反电动势弱、观测信噪比较低影响更明显，高速段则可能受到采样延迟、PLL动态相位滞后、电流采样噪声和参数不匹配的共同影响。

3. 速度估计并非简单随速度升高而更好。0.2–0.4 pu 下速度误差 RMS 较小，而 0.5 pu 下速度误差 RMS 增大到 0.01257 pu，且速度误差曲线出现明显周期性波动。这说明在较高速度下，eSMO/PLL 的速度估计可能受到反电动势波形畸变、数字延迟补偿不足、速度滤波链路相位滞后或机械周期扰动的影响。

4. `eqMag` 随速度升高整体增大，符合反电动势幅值随转速升高而增强的规律；但在 0.5 pu 下，`eqMag` 和 PLL angle-error indicator 的波动也明显增大。这说明高速度下虽然反电动势幅值更强，但观测器输入信号的动态波动也更明显，不能仅用 `eqMag` 均值大小判断观测质量，还需要结合 `thetaErr`、速度误差和最终角度误差综合评价。

5. 后续优化方向不应只调一个固定角度补偿值，而应考虑速度相关的补偿策略。较合理的做法是以 0.1–0.5 pu 稳态短窗口数据为基础，建立 `angleErr` 均值随速度变化的补偿表或拟合函数；同时继续观察去偏置 RMS、速度误差 RMS、`eqMag` 和 `thetaErr`，避免只消除角度均值偏差而放大动态抖动。
