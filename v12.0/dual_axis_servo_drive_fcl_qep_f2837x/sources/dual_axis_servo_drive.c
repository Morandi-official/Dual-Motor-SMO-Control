...
static void applyStoppedCurrentLoopVoltageLimits(MOTOR_Vars_t *pMotor,
                                                 uint16_t motorNum);
static void updateDifferentialDOB(void);
static void resetDifferentialDOB(void);
static inline float32_t getDifferentialIqComp(void);
...
// Common/differential coordination diagnostics and tunable parameters.
// flagSyncPI now enables the differential disturbance observer instead of
// the removed synchronization PI.
#pragma DATA_SECTION(syncPlantGain, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDobBandwidthHz, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDiffKp, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncIqCompLimitPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncIqCompSlewPuPerSec, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncSpeedDeadbandPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncMinRunSpeedPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncCommonSpeedPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncCommonRefPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDiffSpeedPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDiffRefPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDiffErrPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDiffIqFbkPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDiffDisturbanceHatPuPerSec, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncIqCompTargetPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncIqCompPu, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDobState, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDobTsSec, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncDOBActive, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncESMOHealthConfirmMs, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncESMOLiveHealthy, "SYNC_DOB_DATA")
#pragma DATA_SECTION(syncESMOHealthCount, "SYNC_DOB_DATA")
volatile float32_t syncPlantGain = 19.0f;          // (speed pu/s) / Iq pu
volatile float32_t syncDobBandwidthHz = 5.0f;
volatile float32_t syncDiffKp = 0.10f;             // Iq pu / speed pu
volatile float32_t syncIqCompLimitPu = 0.015f;     // first-run safe limit
volatile float32_t syncIqCompSlewPuPerSec = 0.10f;
volatile float32_t syncSpeedDeadbandPu = 0.001f;
volatile float32_t syncMinRunSpeedPu = 0.20f;

volatile float32_t syncCommonSpeedPu = 0.0f;
volatile float32_t syncCommonRefPu = 0.0f;
volatile float32_t syncDiffSpeedPu = 0.0f;
volatile float32_t syncDiffRefPu = 0.0f;
volatile float32_t syncDiffErrPu = 0.0f;
volatile float32_t syncDiffIqFbkPu = 0.0f;
volatile float32_t syncDiffDisturbanceHatPuPerSec = 0.0f;
volatile float32_t syncIqCompTargetPu = 0.0f;
volatile float32_t syncIqCompPu = 0.0f;
volatile float32_t syncDobState = 0.0f;
volatile float32_t syncDobTsSec = 0.0f;
volatile uint16_t syncDOBActive = 0U;
volatile uint16_t syncESMOHealthConfirmMs = 500U;
volatile uint16_t syncESMOLiveHealthy = 0U;
volatile uint16_t syncESMOHealthCount = 0U;
...
typedef struct _ESMO_CompareLog_t_
{
    ...
    uint16_t flags;             // bit0 runMotor, bit1 tripFlagDMC, bit2 takeover,
                                // bit3 QEP index calibrated, bit4 DOB enabled,
                                // bit5 DOB active
    int16_t rcSetpoint_q15;
    ...
    int16_t eqMag_q15;
    int16_t thetaErr_q15;
    int16_t otherSpeed_q15;
    int16_t otherIqFbk_q15;
    int16_t syncDiffSpeed_q15;  // 0.5*(speed0-speed1)
    int16_t syncDiffErr_q15;    // syncDiffSpeed-syncDiffRef
    int16_t syncIqComp_q15;
    int16_t syncDisturbanceHat_q15; // estimated disturbance in speed pu/s
} ESMO_CompareLog_t;
...
// Enables the differential DOB coordination controller; false preserves the
// two independent speed-loop paths with zero cross-coupling compensation.
...
static void resetDifferentialDOB(void)
{
    syncDiffDisturbanceHatPuPerSec = 0.0f;
    syncIqCompTargetPu = 0.0f;
    syncIqCompPu = 0.0f;
    syncDobState = 0.0f;
    syncDOBActive = 0U;
}
...
static void updateDifferentialDOB(void)
{
    ...
    float32_t minSpeed;
    float32_t slewRate;
    float32_t errForControl;
    float32_t stateDerivative;
    float32_t disturbanceLimit;
    float32_t slewStep;
    float32_t delta;
    uint32_t healthConfirmTicks;
    uint16_t operatingReady;
    uint16_t sensorlessUsed;
    uint16_t sensorlessLiveHealthy;
    uint16_t enableDOB;

    syncCommonSpeedPu = 0.5f * (motorVars[0].speed.Speed +
                                      motorVars[1].speed.Speed);
    syncCommonRefPu = 0.5f * (motorVars[0].rc.SetpointValue +
                                    motorVars[1].rc.SetpointValue);
    syncDiffSpeedPu = 0.5f * (motorVars[0].speed.Speed -
                                    motorVars[1].speed.Speed);
    syncDiffRefPu = 0.5f * (motorVars[0].rc.SetpointValue -
                                  motorVars[1].rc.SetpointValue);
    syncDiffErrPu = syncDiffSpeedPu - syncDiffRefPu;
    syncDiffIqFbkPu = 0.5f * (motorVars[0].ptrFCL->pi_iq.fbk -
                                    motorVars[1].ptrFCL->pi_iq.fbk);

    plantGain = __fsat(syncPlantGain, 30.0f, 10.0f);
    bandwidthHz = __fsat(syncDobBandwidthHz, 10.0f, 0.5f);
    iqLimit = __fsat(syncIqCompLimitPu, 0.05f, 0.0f);
    kpDiff = __fsat(syncDiffKp, 0.50f, 0.0f);
    deadband = __fsat(syncSpeedDeadbandPu, 0.02f, 0.0f);
    minSpeed = __fsat(syncMinRunSpeedPu, 0.50f, 0.0f);
    slewRate = __fsat(syncIqCompSlewPuPerSec, 1.0f, 0.0f);
    syncDobTsSec = motorVars[0].Ts *
            (float32_t)motorVars[0].speedLoopPrescaler;

    if((syncDobTsSec <= 0.0f) || (syncDobTsSec > 0.01f))
    {
        syncDobTsSec = 0.001f;
    }

    operatingReady = ((motorVars[0].runMotor == MOTOR_RUN) &&
                      (motorVars[1].runMotor == MOTOR_RUN) &&
                      (motorVars[0].tripFlagDMC == 0U) &&
                      (motorVars[1].tripFlagDMC == 0U) &&
                      (motorVars[0].ptrFCL->lsw == ENC_CALIBRATION_DONE) &&
                      (motorVars[1].ptrFCL->lsw == ENC_CALIBRATION_DONE) &&
                      (absF32(motorVars[0].speed.Speed) >= minSpeed) &&
                      (absF32(motorVars[1].speed.Speed) >= minSpeed)) ? 1U : 0U;

    sensorlessUsed = ((motorVars[0].positionFeedback == POSITION_FEEDBACK_ESMO) ||
                      (motorVars[1].positionFeedback == POSITION_FEEDBACK_ESMO)) ?
                     1U : 0U;
    sensorlessLiveHealthy = 1U;

    if((motorVars[0].positionFeedback == POSITION_FEEDBACK_ESMO) &&
       ((isSensorlessTakeoverActive(&motorVars[0]) != 0U) ||
        (esmoLockAngErrDeg[0] > esmoLockAngErrThrDeg[0]) ||
        (absF32(motorVars[0].esmoSpeedErrPu) > esmoLockSpdErrThrPu[0])))
    {
        sensorlessLiveHealthy = 0U;
    }

    if((motorVars[1].positionFeedback == POSITION_FEEDBACK_ESMO) &&
       ((isSensorlessTakeoverActive(&motorVars[1]) != 0U) ||
        (esmoLockAngErrDeg[1] > esmoLockAngErrThrDeg[1]) ||
        (absF32(motorVars[1].esmoSpeedErrPu) > esmoLockSpdErrThrPu[1])))
    {
        sensorlessLiveHealthy = 0U;
    }

    if((operatingReady == 0U) || (sensorlessLiveHealthy == 0U))
    {
        syncESMOHealthCount = 0U;
        syncESMOLiveHealthy = 0U;
    }
    else if(sensorlessUsed == 0U)
    {
        // QEP operation does not need the sensorless-health confirmation delay.
        syncESMOHealthCount = 0U;
        syncESMOLiveHealthy = 1U;
    }
    else
    {
        healthConfirmTicks = (uint32_t)(((float32_t)syncESMOHealthConfirmMs) *
                                        0.001f / syncDobTsSec);
        if(healthConfirmTicks < 1U)
        {
            healthConfirmTicks = 1U;
        }
        else if(healthConfirmTicks > 65535U)
        {
            healthConfirmTicks = 65535U;
        }

        if((uint32_t)syncESMOHealthCount < healthConfirmTicks)
        {
            syncESMOHealthCount++;
        }
    ...
    // esmoLostLock remains a per-run diagnostic latch. Coordination uses the
    // current, confirmed health state so a recovered startup transient cannot
    // disable the DOB for the rest of an otherwise healthy run.
    enableDOB = ((flagSyncPI == true) &&
                 (operatingReady != 0U) &&
                 (syncESMOLiveHealthy != 0U)) ? 1U : 0U;

    omegaObs = 2.0f * PI * bandwidthHz;
    ...
    if(syncDOBActive == 0U)
    {
        // Initialize dHat = z + omegaObs*omegaDiff to zero at engagement.
        syncDobState = -omegaObs * syncDiffSpeedPu;
        syncDiffDisturbanceHatPuPerSec = 0.0f;
        syncIqCompTargetPu = 0.0f;
        syncIqCompPu = 0.0f;
        syncDOBActive = 1U;
    ...
    // Reduced-order DOB for: d(omegaDiff)/dt = b0*iqDiff + disturbanceDiff.
    // dHat = z + wo*omegaDiff, zDot = -wo*z - wo^2*omegaDiff
    //                                  - wo*b0*iqDiff.
    stateDerivative = -omegaObs * syncDobState -
            omegaObs * omegaObs * syncDiffSpeedPu -
            omegaObs * plantGain * syncDiffIqFbkPu;
    syncDobState += syncDobTsSec * stateDerivative;
    syncDiffDisturbanceHatPuPerSec = syncDobState +
            omegaObs * syncDiffSpeedPu;

    disturbanceLimit = plantGain * 0.05f;
    syncDiffDisturbanceHatPuPerSec =
            __fsat(syncDiffDisturbanceHatPuPerSec,
                   disturbanceLimit, -disturbanceLimit);
    syncDobState = syncDiffDisturbanceHatPuPerSec -
            omegaObs * syncDiffSpeedPu;

    if(absF32(syncDiffErrPu) <= deadband)
    {
        errForControl = 0.0f;
    }
    else if(syncDiffErrPu > 0.0f)
    {
        errForControl = syncDiffErrPu - deadband;
    }
    else
    {
        errForControl = syncDiffErrPu + deadband;
    ...
    // Positive compensation is added to M0 and subtracted from M1.
    syncIqCompTargetPu = -(syncDiffDisturbanceHatPuPerSec / plantGain) -
            kpDiff * errForControl;
    syncIqCompTargetPu = __fsat(syncIqCompTargetPu, iqLimit, -iqLimit);

    slewStep = slewRate * syncDobTsSec;
    delta = syncIqCompTargetPu - syncIqCompPu;
    if(delta > slewStep)
    {
        syncIqCompPu += slewStep;
    }
    ...
    {
        syncIqCompPu = syncIqCompTargetPu;
    }
}
                                  ...
                                  uint16_t motorNum)
{
    ...
    float32_t iqLimit;
    float32_t vsLimitSq;
    float32_t triggerSpeedPu;
    float32_t qepAngleLogPu;
    float32_t qepSpeedLogPu;
    uint16_t gateMask;

    esmoCompareLogCallCount++;

    // Active is a deliberate CCS force-record command. Once active, bypass
    // the arming gates and finish the selected M1 capture deterministically.
    if((esmoCompareLogActive == 0U) &&
       (motorNum != esmoCompareLogMotor))
    {
        esmoCompareLogGateMask = 0x0001U;
        return;
    }

    // In the native QEP control path, ENC_CALIBRATION_DONE already proves
    // that the encoder index/calibration sequence completed. The separate
    // esmoQepIndexCalibrated flag belongs to the eSMO parallel-reference
    // path and is intentionally not maintained while QEP directly controls.
    if((esmoCompareLogActive == 0U) &&
       (pMotor->positionFeedback != POSITION_FEEDBACK_QEP) &&
       (esmoCompareRequireQepIndex != 0U) &&
       (motorNum <= MTR_2) &&
       (esmoQepIndexCalibrated[motorNum] == 0U))
    {
        esmoCompareLogGateMask = 0x0002U;
        return;
    }

    triggerSpeedPu = (pMotor->positionFeedback == POSITION_FEEDBACK_ESMO) ?
            pMotor->esmoSpeedPu : pMotor->speed.Speed;

    if(pMotor->positionFeedback == POSITION_FEEDBACK_QEP)
    {
        qepAngleLogPu = pMotor->ptrFCL->qep.ElecTheta;
        qepSpeedLogPu = pMotor->speed.Speed;
    }
    else
    {
        qepAngleLogPu = pMotor->esmoQepAnglePu;
        qepSpeedLogPu = (motorNum <= MTR_2) ?
                esmoQepSpeedPu[motorNum] : 0.0f;
    }

    gateMask = 0U;
    gateMask |= (pMotor->runMotor != MOTOR_RUN) ? 0x0004U : 0U;
    gateMask |= (pMotor->ptrFCL->lsw != ENC_CALIBRATION_DONE) ? 0x0008U : 0U;
    gateMask |= (absF32(pMotor->rc.SetpointValue) <
                 esmoCompareTriggerCommandPu) ? 0x0010U : 0U;
    gateMask |= (absF32(triggerSpeedPu) <
                 esmoCompareTriggerSpeedPu) ? 0x0020U : 0U;
    esmoCompareLogGateMask = gateMask;
    ...
    esmoCompareLog[esmoCompareLogIndex].flags =
            ...
            ((flagSyncPI == true) ? 16U : 0U) |
            ((syncDOBActive != 0U) ? 32U : 0U);
    ...
    esmoCompareLog[esmoCompareLogIndex].thetaErr_q15 =
            q15Log((pEsmo != (ESMO_Obj *)0) ? pEsmo->thetaErr : 0.0f);
    esmoCompareLog[esmoCompareLogIndex].otherSpeed_q15 =
            q15Log(motorVars[(motorNum == MTR_1) ? MTR_2 : MTR_1].speed.Speed);
    esmoCompareLog[esmoCompareLogIndex].otherIqFbk_q15 =
            q15Log(motorVars[(motorNum == MTR_1) ? MTR_2 : MTR_1].ptrFCL->pi_iq.fbk);
    esmoCompareLog[esmoCompareLogIndex].syncDiffSpeed_q15 =
            q15Log(syncDiffSpeedPu);
    esmoCompareLog[esmoCompareLogIndex].syncDiffErr_q15 =
            q15Log(syncDiffErrPu);
    esmoCompareLog[esmoCompareLogIndex].syncIqComp_q15 =
            q15Log(syncIqCompPu);
    esmoCompareLog[esmoCompareLogIndex].syncDisturbanceHat_q15 =
            q15Log(syncDiffDisturbanceHatPuPerSec);
...
// -----------------------------------------------------------------------------
    ...
    updateSensorlessLockWatch(&motorVars[0], MTR_1);
    ...
    motorVars[0].ptrFCL->pi_iq.ref =
            (motorVars[0].ptrFCL->lsw == ENC_ALIGNMENT) ? 0 :
                    (motorVars[0].ptrFCL->lsw == ENC_WAIT_FOR_INDEX) ?
                            getSensorlessStartupIqRef(&motorVars[0]) :
                            limitSensorlessTakeoverIqRef(&motorVars[0],
                                    __fsat((motorVars[0].pid_spd.term.Out +
                                            getDifferentialIqComp()),
                                           motorVars[0].pid_spd.param.Umax,
                                           motorVars[0].pid_spd.param.Umin));
...
// -----------------------------------------------------------------------------
    ...
    updateSensorlessLockWatch(&motorVars[1], MTR_2);
    ...
    motorVars[1].ptrFCL->pi_iq.ref =
            (motorVars[1].ptrFCL->lsw == ENC_ALIGNMENT) ? 0 :
                    (motorVars[1].ptrFCL->lsw == ENC_WAIT_FOR_INDEX) ?
                            getSensorlessStartupIqRef(&motorVars[1]) :
                            limitSensorlessTakeoverIqRef(&motorVars[1],
                                    __fsat((motorVars[1].pid_spd.term.Out -
                                            getDifferentialIqComp()),
                                           motorVars[1].pid_spd.param.Umax,
                                           motorVars[1].pid_spd.param.Umin));
