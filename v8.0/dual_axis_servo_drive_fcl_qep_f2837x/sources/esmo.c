              ...
              MATH_vec2 *pIabVec)
{
    ...
    // With pll_Ki > 0 the integrator carries the steady-state speed, so the
    // PLL tracks a constant speed with thetaErr -> 0 (a P-only PLL needs a
    // standing phase error of speed/Kp to produce its output). Clamp the
    // integrator to the PLL output range as anti-windup.
    obj->pll_ui = (obj->pll_Ki * obj->thetaErr) + obj->pll_ui;
    obj->pll_ui = __fsat(obj->pll_ui, obj->pll_Umax, obj->pll_Umin);
