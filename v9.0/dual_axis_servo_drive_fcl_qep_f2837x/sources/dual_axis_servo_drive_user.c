...
void initMotorParameters(MOTOR_Vars_t *pMotor, HAL_MTR_Handle mtrHandle)
{
    ...
    if(pMotor == &motorVars[0])
    {
        ...
        pMotor->FCL_params.cmidsqrt3 =
                pMotor->FCL_params.carrierMid * sqrt(3.0);
        ...
        pMotor->currentScale = M1_CURRENT_SENSE_SCALE;
        pMotor->voltageScale = M1_VOLTAGE_SENSE_SCALE;
        pMotor->adcScale = M1_ADC_PU_SCALE_FACTOR;

        pMotor->positionFeedback = M1_POSITION_FEEDBACK;
        pMotor->esmoAngleOffsetPu = M1_ESMO_ANGLE_OFFSET_PU;
        pMotor->esmoPwmScale = 1.0f / pMotor->FCL_params.carrierMid;
        pMotor->esmoAngleDelaySF = 0.5f * MATH_TWO_PI * pMotor->Ts;
        pMotor->esmoForceRunCntMax =
                (uint16_t)(M1_ESMO_FORCE_RUN_SEC * M1_SAMPLING_FREQ);
        pMotor->esmoTakeoverCntMax =
                (uint16_t)(M1_ESMO_TAKEOVER_SEC * M1_SAMPLING_FREQ);
        pMotor->esmoForceSpeed = M1_ESMO_FORCE_SPEED;
    ...
    else if(pMotor == &motorVars[1])
    {
        ...
        // set up current and voltage scaling coefficient
        pMotor->currentScale = M2_CURRENT_SENSE_SCALE;
        pMotor->voltageScale = M2_VOLTAGE_SENSE_SCALE;
        pMotor->adcScale = M2_ADC_PU_SCALE_FACTOR;

        pMotor->positionFeedback = M2_POSITION_FEEDBACK;
        pMotor->esmoAngleOffsetPu = M2_ESMO_ANGLE_OFFSET_PU;
        pMotor->esmoPwmScale = 1.0f / pMotor->FCL_params.carrierMid;
        pMotor->esmoAngleDelaySF = 0.5f * MATH_TWO_PI * pMotor->Ts;
        pMotor->esmoForceRunCntMax =
                (uint16_t)(M2_ESMO_FORCE_RUN_SEC * M2_SAMPLING_FREQ);
        pMotor->esmoTakeoverCntMax =
                (uint16_t)(M2_ESMO_TAKEOVER_SEC * M2_SAMPLING_FREQ);
        pMotor->esmoForceSpeed = M2_ESMO_FORCE_SPEED;
    ...
    initSensorlessEstimator(pMotor);
    resetSensorlessEstimator(pMotor);
...
{
    float32_t piIdUmaxSf = (pMotor == &motorVars[0]) ?
            M1_PI_ID_UMAX_SF : M2_PI_ID_UMAX_SF;
    float32_t piIqUmaxSf = (pMotor == &motorVars[0]) ?
            M1_PI_IQ_UMAX_SF : M2_PI_IQ_UMAX_SF;
    ...
    pMotor->pi_id.Umax = piIdUmaxSf * pMotor->maxModIndex;
    pMotor->pi_id.Umin = -piIdUmaxSf * pMotor->maxModIndex;
    pMotor->pi_id.ref = 0;
    pMotor->pi_id.err = 0;
    pMotor->pi_id.out = 0;
    ...
    pMotor->ptrFCL->pi_iq.Umax = piIqUmaxSf * pMotor->maxModIndex;
    pMotor->ptrFCL->pi_iq.Umin = -piIqUmaxSf * pMotor->maxModIndex;
