...
static void captureESMOCompareLog(MOTOR_Vars_t *pMotor,
                                  uint16_t motorNum);
static inline void updateESMOQepReference(MOTOR_Vars_t *pMotor,
                                          uint16_t motorNum);
static inline void updateESMOQepIndexReference(MOTOR_Vars_t *pMotor,
                                               uint16_t motorNum);
...
// eSMO/QEP short-window compare log. At 500Hz and 96 points it records 0.192s.
...
typedef struct _ESMO_CompareLog_t_
{
    ...
    int16_t rcSetpoint_q15;
    int16_t thetaPllRaw_q15;
    int16_t thetaDelayComp_q15;
    int16_t thetaAfterDelay_q15;
    int16_t esmoAngle_q15;       // final eSMO angle used by FOC
    int16_t qepAngle_q15;
    int16_t rawAngleErr_q15;
    int16_t afterDelayErr_q15;
...
volatile uint16_t esmoCompareLogArmed = 0;
...
static inline void limitSensorlessTakeoverSpeedPid(MOTOR_Vars_t *pMotor)
{
    ...
    limitedOut = limitSensorlessTakeoverIqRef(pMotor, pMotor->pid_spd.term.Out);
                                  ...
                                  uint16_t motorNum)
{
...
#else
    ESMO_Obj *pEsmo = (ESMO_Obj *)pMotor->esmoHandle;
    float32_t thetaPllRawPu;
    float32_t thetaDelayCompPu;
    float32_t thetaAfterDelayPu;
    float32_t rawAngleErrPu;
    float32_t afterDelayErrPu;
    ...
    esmoCompareLog[esmoCompareLogIndex].rcSetpoint_q15 =
            q15Log(pMotor->rc.SetpointValue);
    thetaPllRawPu = (pEsmo != (ESMO_Obj *)0) ?
            normalizePuLocal(ESMO_getAnglePLL((ESMO_Handle)pEsmo) *
                             MATH_ONE_OVER_TWO_PI) : 0.0f;
    thetaDelayCompPu = wrapPuHalfLocal((pMotor->esmoSpeedHz *
                                        pMotor->esmoAngleDelaySF) *
                                       MATH_ONE_OVER_TWO_PI);
    thetaAfterDelayPu = normalizePuLocal(thetaPllRawPu + thetaDelayCompPu);
    rawAngleErrPu = wrapPuHalfLocal(thetaPllRawPu - pMotor->esmoQepAnglePu);
    afterDelayErrPu = wrapPuHalfLocal(thetaAfterDelayPu -
                                      pMotor->esmoQepAnglePu);

    esmoCompareLog[esmoCompareLogIndex].thetaPllRaw_q15 =
            q15Log(thetaPllRawPu);
    esmoCompareLog[esmoCompareLogIndex].thetaDelayComp_q15 =
            q15Log(thetaDelayCompPu);
    esmoCompareLog[esmoCompareLogIndex].thetaAfterDelay_q15 =
            q15Log(thetaAfterDelayPu);
    esmoCompareLog[esmoCompareLogIndex].esmoAngle_q15 =
            q15Log(pMotor->esmoAnglePu);
    esmoCompareLog[esmoCompareLogIndex].qepAngle_q15 =
            q15Log(pMotor->esmoQepAnglePu);
    esmoCompareLog[esmoCompareLogIndex].rawAngleErr_q15 =
            q15Log(rawAngleErrPu);
    esmoCompareLog[esmoCompareLogIndex].afterDelayErr_q15 =
            q15Log(afterDelayErrPu);
...
// -----------------------------------------------------------------------------
    if(isSensorlessControl(&motorVars[0]) != 0U)
    {
        ...
        motorVars[0].speed.ElecTheta = motorVars[0].posElecTheta;
        motorVars[0].speed.Speed = motorVars[0].esmoSpeedPu;
...
// -----------------------------------------------------------------------------
    if(isSensorlessControl(&motorVars[1]) != 0U)
    {
        ...
        motorVars[1].speed.ElecTheta = motorVars[1].posElecTheta;
        motorVars[1].speed.Speed = motorVars[1].esmoSpeedPu;
